...
.Objectif
_Singleton_ garantit qu’une seule instance d’une classe sera créée et fournit un accès uniforme à cet unique objet.

.Structure du Singleton.
[plantuml, target=singleton, format=svg]
----
class Singleton {
...
CAUTION: La création de l'instance est délicate.
...
----
include::{sourcedir}/singleton/MainAsaClass.java[tag=main-as-a-class]
----

== Exemple: Le programme principal comme un Singleton (avec une énumération)
[source,java,indent=0]
----
include::{sourcedir}/singleton/MainAsAnEnum.java[tag=main-as-an-enum]
----

== Le patron de conception Builder
.Objectif
_Builder_ propose une solution pour les cas où la logique de création d’un objet nécessite de nombreux paramètres.
...
[plantuml, target=builder, format=svg]
----
interface Builder {
    +buildPartA() : void
    +buildPartB() : void
...
[source,java,indent=0]
----
include::{sourcedir}/builder/StreetMap.java[tag=streetmap-class]
----

== Exemple: La classe imbriquée Builder de `StreetMap` 2/3
[source,java,indent=0]
----
include::{sourcedir}/builder/StreetMap.java[tag=streetmap-builder]
----

== Exemple: Utilisation du Builder 3/3
[source,java,indent=0]
----
include::{testdir}/builder/BuilderTest.java[tag=streetmap-test]
----
